...
func PresignedURL(objectKey string) (string, string, error) {
	...
	tempObjectKey := fmt.Sprintf("%s/%s", os.Getenv("CS_TEMP"), objectKey)

	object, err := preSigner.PutObject(os.Getenv("CS_BUCKET"), tempObjectKey, 60*3)
	if err != nil {
		return "", "", err
	}
...
func LegitimizeFiles(files []dto.LegitimizeItem) []dto.LegitimizeItem {
	...
	fsS3 := storage.Instance(cs3.Type)
